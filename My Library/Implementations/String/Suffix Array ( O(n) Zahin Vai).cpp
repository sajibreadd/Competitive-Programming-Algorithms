...
const int N = 2000010; // maximum possible length of teh string

char str[N];
int s0[(N / 3) + 10], sa0[(N / 3) + 10];
int n, ar[N], sa[N], lcp[N], bucket[N], mem[N << 2];
    ...
    for (k = 0, l = ((n % 3) == 1), r = 0; r < n; r++){
        ...
        if (l == c) sa[r] = sa0[k++];
            ...
            if (sa12[l] < n0){
                ...
                if ((ar[i] < ar[j]) || (ar[i] == ar[j] && ar[i + 1] < ar[j + 1]) || (ar[i] == ar[j]
                    && ar[i + 1] == ar[j + 1] && s12[sa12[l] - n0 + 1] <= s12[(j /3) + n0]) ) sa[r] = i, l++;
                else sa[r] = j, k++;
            }
        }
    }

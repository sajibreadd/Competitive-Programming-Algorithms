...
int solve(int x, int y, int deep){
   if(!x && !y){ return 1; }
   int use = min(x, (n - deep) / r);
   if(deep + r < n && use){ return solve(x - use, y, deep + use * r); }
   use = min(y, deep / l);
   if(use){ return solve(x, y - use, deep - l * use); }
   return 0;
}
int main(){
...
//    freopen("output.txt", "w", stdout);
   ...
   while(num--){
      ...
      if(t % R){ ps("uh-oh!\n"); }
         ...
         int a = ceil(max(-1.0 * x * t / l, 1.0 * y * t / r));
         x = x * t / R + a * l / R;
         y = y * t / R - a * r / R;
         if(solve(x, -y, 0)){ pi(x - y); nl; }
         else{ ps("uh-oh!\n"); }
      }
   }
   return 0;
